...
export default function AsideC({editar, setEditar, dataVentas, todasVentas,  setDataVentas}){
  ...
  const filtrarDatos = (e)=>{
    ...
    if(e.target.value.trim()=="" ||dataVentas.lenght==0  ){
      ...
      setDataVentas( dataVentas.filter(f=>f._id.includes(e.target.value)))
    }

  }
    return(  <aside className="bg-dark p-4 mx-auto col-12 col-md-2 order-1  ">
   ...
   {/*  <button className="btn btn-danger col-12 col-md-6" type="button">
      Cancelar
    </button>
    <button className="btn btn-success col-12 col-md-6" type="button">
      Guardar
    </button> */}
    </div>
